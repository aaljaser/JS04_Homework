...
let links = [flickerObj.length];

//onsole.log(flickerObj);
...
let links = [];
*/
function surprise(flickerObj)
{ let tittle,date;
    for(let i=0;i<flickerObj["items"].length;i++)
        {tittle=flickerObj["items"][i]["title"];
         date=flickerObj["items"][i]["date_taken"];
console.log("What a surprise in "+tittle+". "+date);
        }
...
function getLink(flickerObj)
{for(let i=0;i<flickerObj["items"].length;i++)
        {links[i]=flickerObj["items"][i].link;
console.log(links[i]);
        }
}
getLink(flickerObj)
